...
import numpy as np
import coconet.core.controller.pynevertemp.pytorch_layers as pyt_l
    ...
    def to_neural_network(self, alt_rep: ONNXNetwork) -> networks.NeuralNetwork:
        ...
        for node in alt_rep.onnx_network.graph.node:
            ...
            if node.op_type == "Relu":
                ...
                network.add_node(nodes.ReLUNode(f"ReLU_{node_index}", in_dim))

            elif node.op_type == "Sigmoid":
                in_dim = tuple(shape_info[node.input[0]][1:])
                network.add_node(nodes.SigmoidNode(f"Sigmoid_{node_index}", in_dim))
                ...
                network.add_node(nodes.FullyConnectedNode(f"Linear_{node_index}", in_dim, in_features,
                ...
                network.add_node(nodes.BatchNormNode(f"BatchNorm_{node_index}", in_dim, num_features, weight,
                ...
                network.add_node(nodes.ConvNode(f"Conv_{node_index}", in_dim, in_channels, out_channels, kernel_size,
                ...
                network.add_node(nodes.AveragePoolNode(f"AveragePool_{node_index}", in_dim, kernel_size, stride,
                ...
                network.add_node(nodes.MaxPoolNode(f"MaxPool_{node_index}", in_dim, kernel_size, stride, padding,
                                                   dilation, ceil_mode))
                ...
                network.add_node(nodes.LRNNode(f"LRN_{node_index}", in_dim, size, alpha, beta, k))
                ...
                network.add_node(nodes.SoftMaxNode(f"Softmax_{node_index}", in_dim, axis))
                ...
                network.add_node(nodes.UnsqueezeNode(f"Unsqueeze_{node_index}", in_dim, axes))
                ...
                network.add_node(nodes.ReshapeNode(f"Reshape_{node_index}", in_dim, shape, allow_zero))
                ...
                axis = node.attribute[0].i - 1
                network.add_node(nodes.FlattenNode(f"Flatten_{node_index}", in_dim, axis))

            elif node.op_type == "Dropout":

                in_dim = tuple(shape_info[node.input[0]][1:])
                ratio = parameters[node.input[1]][0]
                network.add_node(nodes.DropoutNode(f"Dropout_{node_index}", in_dim, ratio))
                ...
                for layer in network.nodes.values():
                    ...
                    if isinstance(layer, nodes.ReLUNode):
                        ...
                        if len(layer.in_dim) == 1 or len(layer.in_dim) == 2:
                            ...
                            new_layer = torch.nn.BatchNorm2d(num_features=layer.num_features,
                                                             eps=layer.eps, momentum=layer.momentum,
                                                             affine=layer.affine,
                                                             track_running_stats=layer.track_running_stats)
                            ...
                            new_layer = torch.nn.BatchNorm3d(num_features=layer.num_features,
                                                             eps=layer.eps, momentum=layer.momentum,
                                                             affine=layer.affine,
                                                             track_running_stats=layer.track_running_stats)
                        ...
                        if len(layer.in_dim) == 2:
                            ...
                            new_layer = torch.nn.Conv1d(layer.in_channels, layer.out_channels, layer.kernel_size,
                            ...
                            new_layer = torch.nn.Conv3d(layer.in_channels, layer.out_channels, layer.kernel_size,
                                                        layer.stride, padding, layer.dilation, layer.groups,
                                                        layer.has_bias)
                        ...
                        if len(layer.in_dim) == 2:
                            ...
                            new_layer = torch.nn.AvgPool1d(layer.kernel_size, layer.stride, padding,
                                                           layer.ceil_mode, layer.count_include_pad)

                        elif len(layer.in_dim) == 3:

                            new_layer = torch.nn.AvgPool2d(layer.kernel_size, layer.stride, padding,
                                                           layer.ceil_mode, layer.count_include_pad)

                        elif len(layer.in_dim) == 4:

                            new_layer = torch.nn.AvgPool3d(layer.kernel_size, layer.stride, padding,
                                                           layer.ceil_mode, layer.count_include_pad)
                        ...
                        if len(layer.in_dim) == 2:
                            ...
                            new_layer = torch.nn.MaxPool1d(layer.kernel_size, layer.stride, padding,
                                                           layer.dilation, layer.return_indices, layer.ceil_mode)

                        elif len(layer.in_dim) == 3:

                            new_layer = torch.nn.MaxPool2d(layer.kernel_size, layer.stride, padding,
                                                           layer.dilation, layer.return_indices, layer.ceil_mode)

                        elif len(layer.in_dim) == 4:

                            new_layer = torch.nn.MaxPool3d(layer.kernel_size, layer.stride, padding,
                                                           layer.dilation, layer.return_indices, layer.ceil_mode)
                        ...
                        new_layer = torch.nn.LocalResponseNorm(layer.size, layer.alpha, layer.beta, layer.k)
                        ...
                        new_layer = torch.nn.Softmax(layer.axis + 1)
                        ...
                        new_layer = pyt_l.Unsqueeze(axes)
                        ...
                        new_layer = pyt_l.Reshape(shape)

                    elif isinstance(layer, nodes.FlattenNode):

                        # We need to scale the axis by one since our representation does not support the batch dimension
                        new_layer = pyt_l.Flatten(layer.axis + 1)

                    elif isinstance(layer, nodes.DropoutNode):

                        new_layer = torch.nn.Dropout(layer.p)

                    else:
                        raise NotImplementedError

                    if new_layer is not None:
                        new_layer.in_dim = layer.in_dim
                        new_layer.out_dim = layer.out_dim
    ...
    def to_neural_network(self, alt_rep: PyTorchNetwork) -> networks.NeuralNetwork:
        ...
        for m in alt_rep.pytorch_network.modules():
            ...
            if isinstance(m, torch.nn.ReLU):
                new_node = nodes.ReLUNode('ReLU_{}'.format(node_index), m.in_dim)

            elif isinstance(m, torch.nn.Sigmoid):
                new_node = nodes.SigmoidNode('Sigmoid_{}'.format(node_index), m.in_dim)
                ...
                if m.bias is not None:
                    ...
                    has_bias = True
                new_node = nodes.FullyConnectedNode('FullyConnected_{}'.format(node_index), m.in_dim, in_features,
                                                    out_features, weight, bias, has_bias)

            elif isinstance(m, torch.nn.BatchNorm1d) or isinstance(m, torch.nn.BatchNorm2d) or \
                ...
                new_node = nodes.BatchNormNode('BatchNorm_{}'.format(node_index), m.in_dim, num_features, weight,
                ...
                new_node = nodes.ConvNode(f"Conv_{node_index}", m.in_dim, in_channels, out_channels, kernel_size,
                                          stride, padding, dilation, groups, has_bias, bias, weight)

            elif isinstance(m, torch.nn.AvgPool1d) or isinstance(m, torch.nn.AvgPool2d) or \
                    isinstance(m, torch.nn.AvgPool3d):
                ...
                new_node = nodes.AveragePoolNode(f"AveragePool_{node_index}", m.in_dim, kernel_size, stride, padding,
                                                 ceil_mode, count_include_pad)

            elif isinstance(m, torch.nn.MaxPool1d) or isinstance(m, torch.nn.MaxPool2d) or \
                    isinstance(m, torch.nn.MaxPool3d):
                ...
                new_node = nodes.MaxPoolNode(f"MaxPool_{node_index}", m.in_dim, kernel_size, stride, padding, dilation,
                                             ceil_mode, return_indices)

            elif isinstance(m, torch.nn.LocalResponseNorm):

                new_node = nodes.LRNNode(f"LRN_{node_index}", m.in_dim, m.size, m.alpha, m.beta, m.k)

            elif isinstance(m, torch.nn.Softmax):

                new_node = nodes.SoftMaxNode(f"SoftMax_{node_index}", m.in_dim, m.dim - 1)

            elif isinstance(m, pyt_l.Unsqueeze):

                axes = tuple([e - 1 for e in m.axes])
                new_node = nodes.UnsqueezeNode(f"Unsqueeze_{node_index}", m.in_dim, axes)

            elif isinstance(m, pyt_l.Reshape):

                shape = m.shape[1:]
                new_node = nodes.ReshapeNode(f"Reshape_{node_index}", m.in_dim, shape)

            elif isinstance(m, pyt_l.Flatten):

                new_node = nodes.FlattenNode(f"Flatten_{node_index}", m.in_dim, m.axis - 1)

            elif isinstance(m, torch.nn.Dropout):

                new_node = nodes.DropoutNode(f"Dropout_{node_index}", m.in_dim, m.p)
